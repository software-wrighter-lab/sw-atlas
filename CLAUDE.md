...
<!-- sw-atlas repo-specific policy below. Keep it OUTSIDE the agentrail
     markers above so `agentrail instructions apply` cannot overwrite it. -->

## What this repo is

One semantic index over every public Software Wrighter artifact, plus a
tiny model that maps a visitor's sentence onto a typed decision about it.
The index holds the facts; the model holds only the language. Docent,
Guide, Librarian, Tutor and Archivist are UI personas of one engine.

Read `docs/plan.md` first; it is the specification. `docs/needle.md` is the
model-architecture assessment. `docs/sw-atlas-research.txt` and
`docs/jev-os-research.txt` are the raw design conversations they were
distilled from -- input, not spec. Where they and the plan disagree, the
plan wins. `docs/sagas.md` is the queue.

`README.md` states the public-facing status, including the scoreboard.
Update it in the same commit as the work it describes.

## Hard constraints

- **The model never emits prose, a URL, or a resource id.** It emits an
  intent, concepts, resource kinds and a confidence; deterministic code
  resolves those against the catalog. A stale model must not be able to
  invent an exhibit.
- **Facts live in the snapshot, language lives in the weights.** If a
  change is factual (status, URL, wording), it rebuilds the index and
  retrains nothing.
- **A0 is never deleted.** The deterministic matcher is a permanent runtime
  class. It is currently the champion at 0.685 against the trained docent's
  0.407, and nothing neural ships until it is beaten by 20 points on the
  same held-out paraphrases.
- **Budgets are tests.** A0 <= 10 MiB, A1 <= 25, A2 <= 64, A3 <= 128,
  A4 <= 256, and the latency table in `docs/plan.md` section 5. Over budget
  is a failing build, not a note in the README.
- **Rust 2024 edition** in every `Cargo.toml`. `sw-checklist` fails
  otherwise.
- **Nothing Python or hand-written JavaScript ships.** The browser runtime
  is Rust compiled to WebAssembly; the snapshot is data. Whether Python may
  exist in the *offline* training pipeline is open decision 1 in
  `docs/plan.md` section 12 and is the repository owner's call -- do not
  settle it in a step.
- **Sibling repositories are read-only.** `../sw-campus`, `../blog` and
  `../../sw-ml-study/moe-microscope` are sources and collaborators, never
  edit targets. Anything they need to change is a work order in
  `docs/<repo>-requests.md` and a line in the session summary.
- **Generated artifacts are never committed.** `build/`, `snapshot/` and
  `checkpoints/` rebuild from committed sources. If a result cannot be
  regenerated by a `just` recipe, it is not a result.

## Measurement discipline

Every claim in this repository cites a row someone can regenerate. A
number that came from a model's impression, a README of another project,
or a plausible estimate is marked as an estimate or it does not go in.
Transferred numbers (MB01, MB01t, CD01, CD01b from moe-microscope) keep
their origin next to them.

When a measurement disappoints, that is the finding. Publish it, keep the
tier below, and say so in the README. The project is a research project;
a negative result that is honestly measured is a delivered step.

## sw-checklist

Run `sw-checklist` before every commit. This repo starts at zero failures
and zero warnings. **Keep it there.** Design to the gate, not the fail
line: <= 25 LOC per function, <= 4 functions per module, <= 4 modules per
crate. When a crate is at 4 modules and the next feature needs a fifth
concern, create the sibling crate first.
...
--all-targets -- -D warnings`, `cargo test`, and `sw-checklist`. Every
command passes before you commit; a red gate means the step is not done.
Then commit, then `agentrail complete`, then push.

Until the workspace exists (Saga 1 step 1), the gate for documentation-only
commits is: every relative link resolves, no trailing whitespace, and every
...
- Step prompts are self-contained but every one begins with "read
  docs/plan.md". Do that.
- Unplanned work goes in via `agentrail insert`, not silently into the
  current step. Future sagas are queued in `docs/sagas.md`.

## Session summary

End every session by reporting: what was pushed, what the next step is,
which choices are waiting on the repository owner, what is blocked, and
what other repositories have been asked for. The last two are how work
orders reach `sw-campus`, `blog` and `moe-microscope`, since this
repository cannot make changes in them.
