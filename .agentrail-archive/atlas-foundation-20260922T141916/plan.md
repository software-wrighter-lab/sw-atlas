...
docs/<repo>-requests.md, not an edit.
...
2. decision-and-requests -- record the owner's no-Python decision (Rust
   and/or sw-MLPL only) in docs/plan.md section 12 and docs/needle.md
   section 5, with its cost; create docs/<repo>-requests.md for
   moe-microscope, sw-campus and blog.

3. corpus-schema -- `atlas-core`: Resource, Concept, Relation, Question,
...
4. ingest-blog -- `atlas-ingest blog ../blog`: 124 posts to Resources from
...
5. ingest-campus -- `atlas-ingest campus ../sw-campus`: snapshot-a.json's
...
6. ingest-metadata -- `atlas-ingest repos` and `atlas-ingest videos`:
...
7. concept-graph -- normalize the union of blog tags/keywords, campus
...
8. coverage-report -- `just report`: the coverage table from docs/plan.md
...
9. results-and-requests -- docs/reference/results.md seeded with the four
   transferred moe-microscope rows (MB01, MB01t, CD01, CD01b) and the empty
   columns Saga 2 will fill; docs/<repo>-requests.md with the three moe-microscope
